...
/*	ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, r->server, APLOGNO(88888)
                 "BEFORE OPENING");*/

	 ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(88888)
                  "BEFORE OPENING");
	...
	const char *path = ap_server_root_relative(tpool, "logs/teste.log");


    rv = apr_file_open(&fp, path, APR_WRITE|APR_CREATE,
                       APR_OS_DEFAULT, tpool);

	apr_file_puts("teste", stdout);
