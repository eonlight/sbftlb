...
static int rr_conn_handler(conn_rec *c) {
	apr_bucket_brigade *brigade = NULL; 
	apr_bucket *bucket = NULL; 
	int seen_eos = 0; 
	int seen_body = 0; 

	const char *debug = ap_server_root_relative(c->pool, "bloom/conn.log");
	
	FILE * debugp = fopen(debug, "w+");
	fprintf(debugp, "IN HANDLER\n");
	fflush(debugp);
...
static void ap_proxy_rr_register_hook(apr_pool_t *p)
{
    ...
    ap_hook_process_connection(rr_conn_handler, NULL, NULL,
                               APR_HOOK_MIDDLE);
}
